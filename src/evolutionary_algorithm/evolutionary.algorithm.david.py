...
def plus_selection(parents, children, mu, matrix):
        combine_parents_children = parents + children
        all_lengths = evaluation(combine_parents_children, matrix)
        paired_tour_length = []
        for i in range(len(combine_parents_children)):
            paired_tour_length.append((combine_parents_children[i], all_lengths[i]))
        n = len(paired_tour_length)
        for i in range(n):
            for j in range(i + 1, n):
                if paired_tour_length[j][1] < paired_tour_length[i][1]:
                    paired_tour_length[i], paired_tour_length[j] = paired_tour_length[j], paired_tour_length[i]
        next_generation = []
        for rank in range(mu):
            next_generation.append(paired_tour_length[rank][0])
...
def generate_algorithm(mu, lam, initial_population, mutation_rate, city_coords, generations):
    first_generation = initialize_first_generation(initial_population, city_coords)
